...
<p align="center">
  <b>AI 浏览器自动化工作台</b><br>
  把一台跑在远端的 Chrome 直播到你的浏览器里，用自己的鼠标键盘直接操控它；<br>
  再对它说一句话，AI 规划、定位、执行，并沉淀成每一步都能自愈的脚本。
</p>

<p align="center">
  <i>Control a remote Chrome from your own browser, then let AI drive it and turn every action into self-healing automation.</i>
</p>
...
<p align="center">
  <sub>左侧是远端 Chrome 直播过来的画面。输入「点击问答按钮」：AI 先规划，再在页面里定位到导航栏的「问答」并点击；页面跳转后，这一步被记入右侧脚本。画面来自本地演示环境，目标站点为 testerhome.com。</sub>
</p>

## 它控制的是一台远程浏览器

这是如意和大多数「AI 操作浏览器」工具最大的不同：被操作的 Chrome 不必在你的电脑上。

- **Chrome 跑在远端**。一台服务器、一台内网机器，或者本机的无头实例。它通过 CDP 被接管，画面经 WebRTC 或 MJPEG 实时推到你的浏览器里。
- **你在本地操控它**。鼠标、滚轮、键盘、中文输入，都从你的浏览器回传到远端执行，就像在操作本地浏览器。帧率、画质、分辨率随时可调，跨网络时附带 TURN 部署脚本。
- **AI 和人共用同一台浏览器**。AI 点到哪，你实时看见；它卡住了，你直接接手，不用停服务，也不用登上远端机器截图。
- **一套链路，多种用法**。云端浏览器、远程调试、无头环境可视化、多个客户端同时观看同一画面。

下面的 AI 对话、操作录制、自愈脚本，都建立在这台远程浏览器之上。
...
- 自动化跑在服务器的无头 Chrome 上，你看不见它在干什么；想手动点一下，只能登上去截图。
...
如意把这几件事放进同一个页面：一台直播到你浏览器里、可以直接操控的远程 Chrome，一个能规划并执行的 AI，一份每一步都带六种定位策略的脚本，以及一键导出的 pytest。

## 它能做什么

### 1. 打开一台看得见、能接管的远程浏览器

远端的 Chrome 画面实时出现在你的浏览器里。你可以直接用鼠标键盘操作它，改地址、开标签页、前进后退，随时从 AI 手里接管。远端可以是服务器、内网机器，也可以是本机的无头实例。传输方式可以在 MJPEG 和 WebRTC H.264 之间切换，帧率、画质、分辨率都能调。
...
- **需要远程操控一台浏览器的人**：Chrome 跑在服务器或内网机器上，想在自己的浏览器里看见它、操作它。
